...
const ListPosts = () => {
  ...
  const filterPostCateByCate = categories?.data.map((item) => {
    const data = postCate?.data.filter(post => post.category.id === item.id)
    return {
      ...item,
      postsCate: data
    }
    ...
    <div>
      ...
      <div className="wrapper_listPost">
        <Row gutter={[20, 10]}>
          ...
          <Col span={13}>
            <Card type="inner" title={categories?.data[id - 1]?.name}>
              {filterPostByIdCate && filterPostByIdCate[id - 1]?.posts.map((data, index) => {
                const color = data?.type.type === "Thắc mắc" ? "#2db7f5" : data?.type.type === "Câu hỏi" ? "#f50" : data?.type.name === "Thảo luận" ? "#108ee9" : "#87d068"
                return (
